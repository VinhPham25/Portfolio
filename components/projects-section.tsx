...
const projects = [
  ...
  },
  {
    ...
    longDescription:
      "Developed in collaboration with the SynapseX research team at Georgia Tech, this BCI system represents a sophisticated machine learning pipeline that translates neural activity into precise cursor control commands. The project leverages Steady-State Visually Evoked Potentials (SSVEP)—a phenomenon where visual stimuli flickering at specific frequencies trigger measurable brainwave responses. I engineered a comprehensive synthetic data generation framework that simulates realistic EEG signals complete with physiological noise characteristics including 1/f pink noise, alpha rhythms, competing frequency responses, and common artifacts like eye blinks and muscle interference. This synthetic pipeline, with configurable difficulty levels, enabled rapid ML experimentation and robust model development without requiring extensive hardware-based data collection upfront. \n\n The classification system I built employs a multi-stage machine learning approach combining signal processing theory with ensemble methods. I designed a Filter Bank Canonical Correlation Analysis (FBCCA) feature extractor that decomposes EEG signals across three frequency bands to capture fundamental frequencies and their harmonics, weighted to match the underlying signal structure (1.0× fundamental, 0.3× second harmonic, 0.1× third harmonic). The preprocessing pipeline I implemented applies bandpass filtering (8-40 Hz), baseline correction, temporal windowing to isolate stable SSVEP periods, and intelligent artifact rejection. For classification, I developed a calibrated voting ensemble that combines Linear Discriminant Analysis with hyperparameter-optimized Support Vector Machines, utilizing probability calibration via Platt scaling to improve confidence estimates. The final system integrates RandomizedSearchCV for automated hyperparameter tuning and achieves reliable four-class directional prediction, all exposed through a FastAPI backend ready for real-time deployment.",
    techStack: {
      ai: ["Scikit-learn (LDA/SVM/Ensemble Methods)", "Filter Bank CCA", "SciPy", "NumPy"],
      backend: ["Python", "FastAPI", "Model Persistence (Joblib)"],
      frontend: []
    },
